...
def init_mixer():
    """嘗試初始化音效，如果失敗就自動切換或停用"""
    for driver in ["wasapi", "directsound", "dummy"]:
        try:
            os.environ["SDL_AUDIODRIVER"] = driver
            pg.mixer.init()
            print(f"音效驅動使用: {driver}")
            return
        except Exception as e:
            print(f"{driver} 初始化失敗:", e)
    # 如果全部失敗
    pg.mixer.quit()
    print("！！沒有可用的音效，聲音功能停用")
...
pg.display.set_icon(pg.image.load(resource_path("image//icon_new.ico")))
